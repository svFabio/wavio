...
describe('useValidateMagicLinkQuery', () => {
  ...
  it('fetches validation when token is provided', async () => {
    ...
    expect(portalApi.validateMagicLink).toHaveBeenCalledWith('valid-token');
  });
});

describe('usePortalAppointmentsQuery', () => {
  it('is disabled when token is empty', () => {
    vi.mocked(portalApi.getPortalAppointments).mockResolvedValue([] as never);
...
describe('usePortalServicesQuery', () => {
  it('is disabled when token is empty', () => {
    vi.mocked(portalApi.getPortalServices).mockResolvedValue([] as never);
...
describe('usePortalAvailableSlotsQuery', () => {
  it('is disabled when token or fecha is empty', () => {
    vi.mocked(portalApi.getPortalAvailableSlots).mockResolvedValue([] as never);
...
describe('useBookAppointmentMutation', () => {
  it('calls bookPortalAppointment', async () => {
    vi.mocked(portalApi.bookPortalAppointment).mockResolvedValue({} as never);
    ...
    await vi.waitFor(() => {
      expect(portalApi.bookPortalAppointment).toHaveBeenCalledWith('token', {
        fecha: '2026-02-01',
        horario: '10:00',
      });
    });
...
describe('useGenerateLinkMutation', () => {
  it('calls generateMagicLink', async () => {
    vi.mocked(portalApi.generateMagicLink).mockResolvedValue({
    ...
    await vi.waitFor(() => {
      expect(portalApi.generateMagicLink).toHaveBeenCalledWith(1);
    });
  });
});
