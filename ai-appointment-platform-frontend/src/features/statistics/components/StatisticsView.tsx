...
export const StatisticsView = ({ overview, revenue, loading }: StatisticsViewProps) => {
    ...
    const statCards = [
        ...
        { label: 'Ingresos del Mes', value: `Bs. ${overview?.ingresosMes || 0}`, icon: DollarSign, gradient: 'from-gradient-cool-from to-gradient-cool-to' },
        { label: 'Clientes Frecuentes', value: overview?.topClientes.length || 0, icon: Users, gradient: 'from-gradient-purple-from to-gradient-purple-to' },
        { label: 'Horarios Activos', value: overview?.horariosPopulares.length || 0, icon: Clock, gradient: 'from-gradient-warm-from to-gradient-warm-to' },
    ];
        ...
        return (
            <div className="space-y-6 p-4 md:p-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[...Array(4)].map((_, i) => <div key={`skeleton-${i}`} className="skeleton h-28 rounded-theme-lg" />)}
                </div>
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                        <LineChart data={revenueFormatted}>
                            ...
                            <Line
                                ...
                                strokeWidth={2.5}
                                dot={{ r: 5, fill: colors.success, strokeWidth: 2, stroke: '#fff' }}
                                activeDot={{ r: 7, stroke: colors.success, strokeWidth: 2 }}
                                name="Ingresos (Bs.)"
                            />
                        </LineChart>
                    </ResponsiveContainer>
                ...
                <div className="card-modern p-5 md:p-6">
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={overview?.horariosPopulares || []} barCategoryGap="20%">
                            ...
                            <Tooltip contentStyle={tooltipStyle} />
                            <Bar dataKey="totalReservas" fill={colors.primary} radius={[8, 8, 0, 0]} name="Reservas">
                                {(overview?.horariosPopulares || []).map((_, index) => (
                                    <Cell key={index} fill={index === 0 ? colors.primary : `${colors.primary}99`} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                    ...
                    <div className="space-y-3">
                        {(overview?.topClientes || []).length === 0 ? (
                            ...
                            overview?.topClientes.map((cliente, index) => (
                                <div key={cliente.telefono} className="flex items-center gap-3 p-3 rounded-xl bg-surface-elevated/50 hover:bg-surface-alt/50 transition-colors">
