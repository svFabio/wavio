...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  const linkClass = (path: string) => `
    ...
    ${isActive(path)
      ? 'bg-surface-elevated text-txt font-medium shadow-sm border border-border'
    ...
    <>
      ...
      `}>
          ...
          >
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-semibold tracking-tight text-txt truncate leading-tight">
                {negocio?.nombre || 'CitasWA'}
              </h2>
              <p className="text-[10px] text-txt-muted font-medium mt-0.5 truncate">{usuario?.nombre || 'Asistente IA'}</p>
            </div>
          </button>
        </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto mt-1">
          <p className="px-3 py-1.5 text-[10px] text-txt-muted uppercase tracking-wider font-semibold">General</p>
          ...
          {isAdmin && (
            <>
              ...
              <p className="px-3 py-1.5 text-[10px] text-txt-muted uppercase tracking-wider font-semibold">Administracion</p>
        ...
        <div className="p-3 border-t border-border bg-surface-elevated/50">
          ...
          <p className="text-center text-[9px] text-txt-muted font-mono mt-2.5 bg-surface-elevated border border-border py-0.5 px-2 rounded-full w-max mx-auto">v1.0.0 PRO</p>
        </div>
      </aside>
