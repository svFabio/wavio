...
import { z } from 'zod';
import { loginConGoogle, loginConEmail, registrarConEmail, me, updateAvatar, deleteAvatar, updateNombre, googleMobileStart, googleMobileCallback, mobileTokenPoll } from '../controllers/auth.controller';
...
const emailAuthSchema = z.object({
  ...
  password: z.string({ message: 'Contraseña requerida' }).min(6, 'La contraseña debe tener al menos 6 caracteres')
});
...
router.put('/me/avatar', verificarToken, validateBody(z.object({ image: z.string() })), updateAvatar);
router.delete('/me/avatar', verificarToken, deleteAvatar);
router.patch('/me/nombre', verificarToken, validateBody(z.object({ nombre: z.string().min(2, 'El nombre debe tener al menos 2 caracteres') })), updateNombre);
