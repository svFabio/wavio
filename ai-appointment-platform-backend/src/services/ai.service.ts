...
export const procesarMensajeConIA = async (
  ...
  try {
    ...
    if (intencion === 'AGENDAR') respuestaRespaldo = 'Entendido, quieres agendar. ¿Para qué fecha?';
      ...
      respuestaRespaldo =
        'Disculpa, tuve un problema de conexión con mi cerebro 🤖. ¿Podrías decirme para qué día buscas cita?';
...
export const detectarIntencionSimple = (mensaje: string): ResultadoIA['intencion'] => {
  ...
  if (
    /hola|buenos|buenas|quiero|necesito|me gustaría|quisiera|agendar|cita|reserva|turno/.test(
      textoLower,
    )
  ) {
    return 'AGENDAR';
