...
export const configuracionService = {
    async getConfiguracion(negocioId: number) {
        return configuracionRepository.getOrCreateByNegocioId(negocioId);
    },

    async updateConfiguracion(negocioId: number, data: Record<string, unknown>) {
        const { trigger, mensajeBienvenida, mensajeConfirmacion, servicios, horarios, cobrarAdelanto, porcentajeAdelanto } = data;
        ...
        if (servicios !== undefined) {
            if (!Array.isArray(servicios) || !servicios.every((s: unknown) => s !== null && typeof s === 'object' && 'nombre' in s && typeof (s as Record<string, unknown>).nombre === 'string' && 'precio' in s && typeof (s as Record<string, unknown>).precio === 'number')) {
        ...
        if (mensajeConfirmacion !== undefined) updateData.mensajeConfirmacion = mensajeConfirmacion as string;
