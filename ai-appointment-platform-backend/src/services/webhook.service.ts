...
import { procesarMensajeConIA, ContextoConversacion } from '../services/aiService';
            ...
            for (const change of entry.changes) {
                ...
                if (messages && messages.length > 0) {
                    ...
                    try {
                        ...
                        if (!negocio) {
                            logger.warn({ phoneNumberId }, '[Webhook] Mensaje para phoneNumberId no registrado');
                            continue;
                        }
                        ...
                        const contexto: ContextoConversacion = sesion
                            ? { estado: sesion.estado as ContextoConversacion['estado'], datos: (sesion.datos as Record<string, unknown>) || {}, intentosAclaracion: 0 }
                        ...
                        logger.error({ err: msgErr, message }, '[Webhook] Error procesando mensaje individual');
                    }
                    }
                }
