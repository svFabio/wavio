...
export const statisticsRepository = {
    ...
    async getCitasConfirmadasMonto(negocioId: number, startOfMonth: Date, endOfMonth: Date) {
    ...
    async getTopClientes(negocioId: number) {
        return prisma.$queryRaw<{ clienteTelefono: string; clienteNombre: string | null; totalCitas: number }[]>`
            SELECT 
                c."clienteTelefono",
    ...
    async getHorariosPopulares(negocioId: number) {
    ...
    async getUltimosComentarios(negocioId: number) {
        return prisma.cita.findMany({
    ...
    async countCitasPorOrigen(negocioId: number, startOfMonth: Date, endOfMonth: Date, origen: string): Promise<number> {
        return prisma.cita.count({
            where: { negocioId, fecha: { gte: startOfMonth, lte: endOfMonth }, estado: { not: 'CANCELADA' }, origen }
        });
    },

    async getCitasIngresos(negocioId: number, startDate: Date): Promise<{ fecha: Date, monto: number }[]> {
        return prisma.cita.findMany({
